...
async def get_all_results(db: AsyncSession = Depends(get_db)):
    ...
    for score, er, doc in rows:
        output.append({
            ...
            "f1_score": float(score.f1_score) if score.f1_score is not None else None,
            "teds_score": float(score.teds_score) if score.teds_score is not None else None,
            "grits_top": float(score.grits_top) if score.grits_top is not None else None,
            "grits_con": float(score.grits_con) if score.grits_con is not None else None,
            "grits_loc": float(score.grits_loc) if score.grits_loc is not None else None,
...
async def export_results_csv(
    ...
    for score, er, doc in rows:
        writer.writerow([
            ...
            float(score.precision) if score.precision is not None else "",
            float(score.recall) if score.recall is not None else "",
            float(score.f1_score) if score.f1_score is not None else "",
            float(score.teds_score) if score.teds_score is not None else "",
            float(score.grits_top) if score.grits_top is not None else "",
            float(score.grits_con) if score.grits_con is not None else "",
            float(score.grits_loc) if score.grits_loc is not None else "",
        ])
